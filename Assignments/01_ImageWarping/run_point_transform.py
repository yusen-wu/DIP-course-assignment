...
def point_guided_deformation(image, source_pts, target_pts, alpha=1.0, eps=1e-8):
    ...
    for i in range(h):
        for j in range(w):
            ...
            v = np.array([j,i],dtype=np.float64)

            weights = np.zeros(n)

            for k in range(n):
                dx = v[0] - target_pts[k,0]
                dy = v[1] - target_pts[k,1]
                dist2 = dx*dx + dy*dy + eps
                weights[k] = 1.0/(dist2**(alpha/2))

            w_sum = np.sum(weights)

            p_star = np.sum(weights[:,None]*target_pts,axis=0)/w_sum
            q_star = np.sum(weights[:,None]*source_pts,axis=0)/w_sum
            ...
            for k in range(n):
                A += weights[k]*np.outer(p_hat[k],p_hat[k])
                B += weights[k]*np.outer(p_hat[k],q_hat[k])

            M = np.linalg.pinv(A) @ B

            src_pos = M @ (v - p_star) + q_star
