...
class ColmapDataset(Dataset):
    def __init__(self, data_path, downsample_factor=8, maximum_pts_num=None,
        ...
        if maximum_pts_num is not None and maximum_pts_num < len(self.points3D_xyz):
            indices = torch.linspace(
                0,
                len(self.points3D_xyz) - 1,
                steps=maximum_pts_num
            ).long()
        ...
        if maximum_image_num is not None and maximum_image_num < len(self.image_paths):
            self.image_paths = self.image_paths[:maximum_image_num]
            self.camera_data = self.camera_data[:maximum_image_num]
