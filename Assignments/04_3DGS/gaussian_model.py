...
def knn_points(points1: torch.Tensor, points2: torch.Tensor, K: int):
    """Small pure-PyTorch KNN fallback for scale initialization."""
    dists = torch.cdist(points1, points2, p=2) ** 2
    knn = torch.topk(dists, k=K, dim=-1, largest=False, sorted=True)
    return knn.values, knn.indices, None
