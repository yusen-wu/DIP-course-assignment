...
def create_mask_from_points(points, img_h, img_w):
    ...
    if len(points) < 3:
        return mask  # Not a valid polygon
    img = Image.fromarray(mask)
    draw = ImageDraw.Draw(img)
    draw.polygon(points.tolist(), fill=255)
    mask = np.array(img) 
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    foreground_lap = F.conv2d(foreground_img, lap_kernel, padding=1, groups=3)
    blended_lap = F.conv2d(blended_img, lap_kernel, padding=1, groups=3)

    fg_mask_3 = foreground_mask.expand(-1, 3, -1, -1)
    bg_mask_3 = background_mask.expand(-1, 3, -1, -1)

    foreground_values = foreground_lap[fg_mask_3 > 0.5]
    blended_values = blended_lap[bg_mask_3 > 0.5]

    loss = ((foreground_values - blended_values) ** 2).mean()
